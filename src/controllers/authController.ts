...
// remove below code before depoyment
...
export const register = async (req: Request, res: Response) => {
    try {
        ...
        if (IS_DEV) {
            ...
            // Return tokens immediately — no verify step needed
            const accessToken = generateAccessToken(user.id);
            const refreshToken = generateRefreshToken(user.id);
            await updateRefreshToken(user.id, refreshToken);
            ...
            res.status(201).json({
                success: true,
                message: "Account created (dev: auto-verified)",
                data: {
                    user: { ...safeUser, isVerified: true },
                    accessToken,
                    refreshToken,
...
export const sendLoginOTP = async (req: Request, res: Response) => {
    try {
        ...
        if (IS_DEV) {
            // Log OTP to console instead of emailing
        ...
        res.status(200).json({
            success: true,
            message: IS_DEV ? "Login code logged to server console (dev mode)" : "Login code sent to your email",
            data: {
                requiresTwoFactor: user.twoFactorEnabled,
                // Expose OTP in response body during dev so the app can prefill it
                ...(IS_DEV && { devOTP: loginOTP }),
            },
        });
    } catch (error) {
        console.error("Send login OTP error:", error);
